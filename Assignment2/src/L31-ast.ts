...
const parseClassExp = (fields: Sexp, methods: Sexp[]): Result<ClassExp> =>{
    ...
    return isArray(fields) && allT(isString, fields) && isGoodBindings(methods[0]) ? bind(mapResult(parseL31CExp, methods[0]),
                                (cexps: CExp[]) => makeOk(makeClassExp(map(makeVarDecl, fields),zipWith(makeBinding,vars ,cexps)))):
      makeFailure('Invalid vars for ClassExp');
}
...
const unparseClassExp = (ce: ClassExp): string =>
        `(${map((p: VarDecl) => p.var, ce.fields).join(" ")}) (${map((b: Binding) => `${unparseL31(b.val)}`, ce.methods).join(" ")})`
...
export const unparseL31 = (exp: Program | Exp): string =>
    ...
    isClassExp(exp) ? `(class ${unparseClassExp(exp)})`:
    exp;
