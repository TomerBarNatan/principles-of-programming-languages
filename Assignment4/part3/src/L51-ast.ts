...
import { typeofExp } from "./L51-typeinference";
import { isTVar, makeFreshTVar, makeTVar, parseTExp, unparseTExp, TVar, TExp, parseTE, isTExp } from './TExp51';
...
const parseGoodClassExp = (typeName: Sexp, varDecls: Sexp, bindings: Sexp): Result<ClassExp> => {
    if (!isArray(varDecls)) 
        return makeFailure('first Malformed fields in "class" expression');
    if (!isGoodBindings(bindings)) 
        return makeFailure('seconed Malformed methodss in "class" expression');
    if(!isString(typeName)){
        const temp = parseSExp(typeName)
        return makeFailure(`Im here!`);
        
    }
    const fields : Result<VarDecl[]> = mapResult(parseVarDecl, varDecls);

    const app_names: Result<VarDecl[]> = mapResult(b => parseVarDecl(b[0]), bindings);
    const apps_vals : Result<CExp[]> = mapResult(binding => parseL5CExp(second(binding)), bindings);
    const methods : Result<Binding[]> = bind(app_names, (nms: VarDecl[]) => bind( apps_vals, (vs: CExp[]) => makeOk(zipWith(makeBinding, nms, vs))));
    return bind(fields, (fd : VarDecl[]) => bind(methods, (mtd: Binding[]) =>  makeOk(makeClassExp(makeTVar(typeName),fd,mtd))));
}
...
const recParsedToClassExps = (p: any, classes: ClassExp[]): ClassExp[] =>
    isNumExp(p) ? [] :
    isBoolExp(p) ? [] :
    isStrExp(p) ? [] :
    isPrimOp(p) ? [] :
    ...
    isProgram(p) ? p.exps.map(x => recParsedToClassExps(x,[])) :
    // L51
    isClassExp(p) ? p : 
    isLitExp(p) ? [] :
    isSetExp(p) ? [] :
...
export const parsedToClassExps = (p: Parsed): ClassExp[] => {
    return  isProgram(p)? recParsedToClassExps(p,[]) : isDefineExp(p) && isClassExp(p.val) ? [p.val]: isClassExp(p)? [p]: recParsedToClassExps(p,[]) ;
}
