...
import { Result, makeFailure, makeOk, bind, safe2, zipWithResult, mapResult, isFailure } from "../shared/result";
...
let all_classes: A.ClassExp[] = [];
export const makeTEnvFromClasses = (parsed: A.Parsed): E.TEnv => {
    // TODO makeTEnvFromClasses
    all_classes = A.parsedToClassExps(parsed).filter(x => !isArray(x));
    if(all_classes.length === 0)
        return E.makeEmptyTEnv();

    const types : T.ClassTExp[] = all_classes.map(x => A.classExpToClassTExp(x));
    const names : string[] = types.map(x => x.typename);
    let newEnv = E.makeExtendTEnv(names,types,E.makeEmptyTEnv());
    // const meth = exp.methods.map(x=> x.val);
    // if(! allT(A.isProcExp,meth))
    //     return makeFailure("impossible");
    // const  func_type: Result<T.TExp[]> = mapResult(x=> typeofProc(x,E.makeExtendTEnv(exp.fields.map(x => x.var),exp.fields.map(x=>x.texp),tenv)),meth);
    // const tempEnv = bind(func_type, ft => makeOk(E.makeExtendTEnv(exp.methods.map(x => x.var.var),ft,tenv)))
    return newEnv;
}
...
export const typeofApp = (app: A.AppExp, tenv: E.TEnv): Result<T.TExp> => {
    if(A.isVarRef(app.rator) && !(isFailure(bind(E.applyTEnv(tenv,app.rator.var), t => T.isClassTExp(t)? makeOk(""): makeFailure("") )))){
        return bind(E.applyTEnv(tenv,app.rator.var), t=> T.isClassTExp(t) && A.isLitExp(app.rands[0]) && V.isSymbolSExp(app.rands[0].val) && t.methods.map(x=>x[0]).indexOf(app.rands[0].val.val) > -1 && T.isTExp(t.methods[t.methods.map(x=>x[0]).indexOf(app.rands[0].val.val)][1])? 
        makeOk(t.methods[t.methods.map(x=>x[0]).indexOf(app.rands[0].val.val)][1])
        : makeFailure("NOT A CLASS"))
    }
    if(app.rands.length === 1 && A.isLitExp(app.rands[0]) && V.isSymbolSExp(app.rands[0].val)){
        const sym: string = app.rands[0].val.val;
        let find : boolean = false;
        for(let i = 0; i< all_classes.length && !find; i++){
            let cls: A.ClassExp = all_classes[i];
            let methods_names = cls.methods.map(x=>x.var.var);
            if(methods_names.indexOf(sym) > -1){
                const meth = cls.methods[methods_names.indexOf(sym)].val;
                return A.isProcExp(meth)? typeofProc(meth,E.makeExtendTEnv(cls.fields.map(x=>x.var),cls.fields.map(x=>x.texp),tenv)): makeFailure("NOT A PROC");
            }
        }
    }
...
const typeofProgramExps = (exp: A.Exp, exps: A.Exp[], tenv: E.TEnv): Result<T.TExp> => {
    if(A.isDefineExp(exp)){
        return bind(typeofExps([exp.val], E.makeExtendTEnv([exp.var.var],[T.makeFreshTVar()],tenv)), res => typeofProgramExps(first(exps),rest(exps),E.makeExtendTEnv([exp.var.var],[res],tenv)));
    }
    if(exps.length > 0){
...
export const typeofLit = (exp: A.LitExp): Result<T.TExp> =>
    V.isCompoundSExp(exp.val)? makeOk(T.makePairTExp()) : V.isSymbolSExp(exp.val) ? makeOk(T.makeSymbolTExp(exp.val)): makeOk(T.makeSymbolTExp());
...
export const typeofClass = (exp: A.ClassExp, tenv: E.TEnv): Result<T.TExp> => {
    ...
    if(isFailure(E.applyTEnv(tenv,exp.methods[0].var.var))){
        const  func_type: Result<T.TExp[]> = mapResult(x=> typeofProc(x,E.makeExtendTEnv(exp.fields.map(x => x.var),exp.fields.map(x=>x.texp),tenv)),meth);
        const tempEnv: Result<E.ExtendTEnv> = bind(func_type, ft => makeOk(E.makeExtendTEnv(exp.methods.map(x => x.var.var),ft,tenv)))
        return bind(tempEnv, te => typeofExp(exp,te));
    }
    let methods: Result<[string,T.TExp][]> = makeOk([]);
    for(let i = 0; i< meth.length; i++){
        methods = bind(E.applyTEnv(tenv,exp.methods[i].var.var), ty=> bind(methods, m=> makeOk(m.concat([[exp.methods[i].var.var,ty]]))));
    }
    // const methods = mapResult() 
    const cls = bind(methods, m =>makeOk(T.makeClassTExp(exp.typeName.var,m)));
    const ret = bind(cls, cls2=> makeOk(T.makeProcTExp(exp.fields.map(x=>x.texp),cls2)));
    return ret;
};